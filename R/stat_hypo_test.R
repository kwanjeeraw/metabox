...
if(length(independent_factor_name)==0){
  ...
  if(length(unique(factor_name))>2){
    return("Sorry. Cannot support more than two way design cases now.")
  }else{
    ...
    if(is.null(repeated_factor_name)){# t test, ANOVA, two way ANOVA, sampleID should be identical to each other.
      if(sum(duplicated(p$sampleID))>0){
        return("sampleID should be identical to each other if your design is independent design.")
      }
    }else if(!is.null(independent_factor_name)){# Mixed factor ANOVA.
      if(sum(as.numeric(by(p$sampleID,p[,repeated_factor_name],function(x){
        sum(duplicated(x))
      })))>0){
        stop("sampleID conflicts with your study design.")
      }
    }# others should be OK.
        ...
        if(length(factor_name[!factor_name%in%repeated_factor_name])==1 & length(repeated_factor_name)==0 & (length(unique(dta[,2]))>2)){#oneway ANOVA.
          ...
          if(!length(unique(dta$variable2))==2){ # if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            variable1_simple_main_effect = by(dta,dta$variable1,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_one_way_ANOVA(data = e[dta$variable1==unique(x$variable1),],data2 = x,i=3,sudo_matrix=sudo_matrix2,factor_name,cl) # the 3rd column is group.
              return(result)
            })
            ...
            variable1_simple_main_effect = by(dta,dta$variable1,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_t_test(e[dta$variable1==unique(x$variable1),],data2 = x,3,cl) # the 3rd column is group.
              return(result)
          ...
          if(!length(unique(dta$variable1))==2){# if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            variable2_simple_main_effect = by(dta,dta$variable2,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_one_way_ANOVA(data = e[dta$variable2==unique(x$variable2),],data2 = x,i=2,sudo_matrix=sudo_matrix1,factor_name,cl) # the 3rd column is group.
              return(result)
            })
            ...
            variable2_simple_main_effect = by(dta,dta$variable2,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_t_test(e[dta$variable2==unique(x$variable2),],x, i=2,cl) # 2nd column is group.
              return(result)
            })
            for(i in 1:length(variable2_simple_main_effect)){
              colnames(variable2_simple_main_effect[[i]]) = paste0(levels(dta$variable2)[i],":_",colnames(variable2_simple_main_effect[[i]]))
          ...
          result = stat_paired_t_test(data = e,data2 = dta,i = 2,cl) # 2nd column is the group
          f$'outlier exist?' = stat_test_outlier(e,f,p,factor_name)
          result = data.frame(f,result,check.names = F)
          result_stat = matrix(nrow = ncol(e),ncol = 1 + 1 + (length(unique(dta[,2])))* 2) # global mean, sd. and mean and sd for each group.
          for(i in 1:ncol(e)){
          ...
          if(!length(unique(dta$repeated2))==2){ # if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            repeated1_simple_main_effect = by(dta,dta$repeated1,FUN = function(x){ #repeated1 simple main effect
              ...
              result = stat_one_way_repeated_ANOVA(data = e[dta$repeated1==unique(x$repeated1),],data2 = x,i=3,sudo_matrix=sudo_matrix2,factor_name,cl) # the 3rd column is group.
              return(result)
            })
            ...
            repeated1_simple_main_effect = by(dta,dta$repeated1,FUN = function(x){ #repeated1 simple main effect
              ...
              result = stat_paired_t_test(data = e[dta$repeated1==unique(x$repeated1),],data2 = x,3,cl) # the 3rd column is group.
              return(result)
            })# returns two length of list. Ordered by levels of INDICES in by().
            for(i in 1:length(repeated1_simple_main_effect)){
              colnames(repeated1_simple_main_effect[[i]]) = paste0(levels(dta$repeated1)[i],":_",colnames(repeated1_simple_main_effect[[i]]))
          ...
          if(!length(unique(dta$repeated1))==2){# if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            repeated2_simple_main_effect = by(dta,dta$repeated2,FUN = function(x){ #repeated1 simple main effect
              ...
              result = stat_one_way_repeated_ANOVA(data = e[dta$repeated2==unique(x$repeated2),],data2 = x,i=2,sudo_matrix=sudo_matrix1,factor_name,cl) # the 3rd column is group.
              return(result)
            })
            ...
            repeated2_simple_main_effect = by(dta,dta$repeated2,FUN = function(x){ #repeated1 simple main effect
              ...
              result = stat_paired_t_test(e[dta$repeated2==unique(x$repeated2),],x, i=2,cl) # 2nd column is group.
              return(result)
            })
            for(i in 1:length(repeated2_simple_main_effect)){
              colnames(repeated2_simple_main_effect[[i]]) = paste0(levels(dta$repeated2)[i],":_",colnames(repeated2_simple_main_effect[[i]]))
         ...
         #
          ...
          if(!length(unique(dta$repeated1))==2){ # if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            variable1_simple_main_effect = by(dta,dta$variable1,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_one_way_repeated_ANOVA(data = e[dta$variable1==unique(x$variable1),],
                                                   data2 = x,i=3,sudo_matrix=sudo_matrix2,factor_name,cl=cl) # the 3rd column is group.
              return(result)
            })
            for(i in 1:length(variable1_simple_main_effect)){
              colnames(variable1_simple_main_effect[[i]]) = paste0(levels(dta$variable1)[i],":_",colnames(variable1_simple_main_effect[[i]]))
            ...
            variable1_simple_main_effect = by(dta,dta$variable1,FUN = function(x){ #variable1 simple main effect
              # x = dta[dta$variable1=="drug 2",]
              result = stat_paired_t_test(e[dta$variable1==unique(x$variable1),],data2 = x,3,cl) # the 3rd column is group.
              return(result)
            })# returns two length of list. Ordered by levels of INDICES in by().
            for(i in 1:length(variable1_simple_main_effect)){
              colnames(variable1_simple_main_effect[[i]]) = paste0(levels(dta$variable1)[i],":_",colnames(variable1_simple_main_effect[[i]]))
          ...
          if(!length(unique(dta$variable1))==2){# if TRUE, then use ANOVA on main effect post hoc analysis.
            ...
            repeated1_simple_main_effect = by(dta,dta$repeated1,FUN = function(x){ #variable1 simple main effect
              # x = dta[dta$repeated1==levels(dta$repeated1)[1],]
              result = stat_one_way_ANOVA(data = e[dta$repeated1==unique(x$repeated1),],data2 = x,i=2,sudo_matrix=sudo_matrix1,factor_name,cl) # the 3rd column is group.
              return(result)
            })
            for(i in 1:length(repeated1_simple_main_effect)){
              colnames(repeated1_simple_main_effect[[i]]) = paste0(levels(dta$repeated1)[i],":_",colnames(repeated1_simple_main_effect[[i]]))
            ...
            repeated1_simple_main_effect = by(dta,dta$repeated1,FUN = function(x){ #variable1 simple main effect
              ...
              result = stat_t_test(e[dta$repeated1==unique(x$repeated1),],x, i=2,cl) # 2nd column is group.
              return(result)
            })
            for(i in 1:length(repeated1_simple_main_effect)){
              colnames(repeated1_simple_main_effect[[i]]) = paste0(levels(dta$repeated1)[i],":_",colnames(repeated1_simple_main_effect[[i]]))
          ...
          return("Your Design is too complex that we couldn't analysis. If you have any question, please contact Sili: slfan@ucdavis.edu")
        }
