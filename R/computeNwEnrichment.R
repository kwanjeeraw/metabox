...
#'@usage computeNwEnrichment(edgelist, nodelist, annotation, pval, fc, pcol, internalid, method, size, hasatt)
...
#'@export
computeNwEnrichment <- function(edgelist, nodelist, annotation="pathway", pval, fc=NULL, pcol=NULL, internalid = TRUE, method="reporter", size=c(3,500), hasatt=FALSE) UseMethod("computeNwEnrichment")
#'@export
computeNwEnrichment.default <- function (edgelist, nodelist, annotation="pathway", pval, fc=NULL, pcol=NULL, internalid = TRUE, method="reporter", size=c(3,500), hasatt=FALSE){
  out <- tryCatch(
    {
      ...
      if(tolower(annotation) == 'pathway' && foundDb()){#pathway enrichment
        ...
        if(!is.null(unlist(annols))){#found annotation
          ...
          if(flagdf){#keep input data
            if(hasatt){
              nodelist = merge(nodelist[,1:5],datinput,by.x='gid',by.y='grinn',all.x=TRUE)
            }else{
